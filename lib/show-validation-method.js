Meteor.methods({
	finalValidateShow: function(showID) {
		...
		if(show.type == "Bandemonium") {
			priority = "BND ";
			var bandemoniumPriorityNumber = (12 - maxTerms);
			if(bandemoniumPriorityNumber < 10) {
				priority += "0" + bandemoniumPriorityNumber;
			} else {
				priority += bandemoniumPriorityNumber;
			}
			priority += maxYear;
		}
		if(show.type == "Standard") {
			if(maxTerms >= 9) {
				if(maxTerms >= 12) maxTerms = 12;
				priority = "A" + (12 - maxTerms);
			} else {
