Meteor.methods({
	...
	numberOfActiveDjs: function() {
		var showCursor = Shows.find({completed: {$exists: true}});
		var netids = [];
		showCursor.forEach(function(show) {
			for(var i = 0; i < show.djs.length; i++) {
				if(netids.indexOf(show.djs[i]) == -1) {
					netids.push(show.djs[i]);
				}
			}
		});
	...
	numberOfNewActiveDjs: function() {
		var includeDjsArray = [];
		var netids = [];
		var shows = Shows.find({completed: {$exists: true}});
		shows.forEach(function(show) {
			var thisShowDjs = show.djs;
			for(var i = 0; i < thisShowDjs.length; i++) {
				if(netids.indexOf(thisShowDjs[i]) == -1) {
					var djObject = DJs.findOne({netid: thisShowDjs[i]})
					if(djObject.terms == 0) {
						includeDjsArray.push(thisShowDjs[i]);
					}
					netids.push(thisShowDjs[i]);
				}
			}
		})
		return includeDjsArray.length;
	}
});
